...
const OptionsContent: React.FC<{ schemaId: string }> = ({ schemaId }) => {
  const [SelectedScreenType, setSelectedScreenType] = useState<"options" | 'folder'>("options")
    ...
    <div className="overflow-hidden border-none rounded-md  shadow-lg outline-none min-w-[170px] ">
      {
        SelectedScreenType === "options" ? (
          <>
            ...
            <div onClick={() => setSelectedScreenType("folder")} className="group flex cursor-pointer items-center px-1 py-1.5 text-sm text-gray-200 hover:bg-gray-600 hover:text-gray-200">
          ...
          : (
            <>
              <div onClick={() => setSelectedScreenType("options")} className="mt-1 mb-2 flex items-center justify-start px-1  pb-1 text-xs font-bold uppercase text-gray-400">
                <i className="fa-solid fa-chevron-left me-2"></i> Move to Folder
              </div>
              {
                isPending ? (
                  <div className="wrapper my-3"> <Spinner size={"sm"} /> </div>
                ) :
                  folders?.data.data?.map((elem: FolderType) => (
                    <div key={elem._id} onClick={() => handleFolderclick(elem._id)} className="group flex cursor-pointer items-center px-1 py-1.5 text-sm text-gray-200 hover:bg-gray-600 hover:text-gray-200">
