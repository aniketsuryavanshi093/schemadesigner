...
const AccordianTitle: React.FC<{ table: Table; isOpen: string }> = ({
  table,
  isOpen,
}) => {
    ...
    <div className="flex relative justify-between gap-2 w-full directionlefttoright accordianheader items-center mx-2">
      {table?.isEditing ? (
        <input
          ...
          className="w-4/5 ps-3 z-[100] h-[1.8rem] text-base "
          onChange={(e) => {
            e.preventDefault();
      ...
      <div className="flex justify-between items-center gap-2">
        {table?.isEditing ? (
          ...
          <Button
            ...
            style={{ padding: " 15px 11px " }}
            className={` ${
              isOpen === table.tableIndex && "editbtnovered"
            } w-5 h-6 min-w-8 gap-0   editbtn`}
            onClick={handleEdit}
          >
        ...
        <ThreeDotBtn isOutsidebtn table={table} />
      </div>
    </div>
  );
};
