...
const CreateNewDaigramModal: React.FC<{ isOpen: boolean, setIsopen: (boolean: boolean) => void }> = ({ isOpen, setIsopen }) => {
    const [isPending, startTransition] = useTransition();
    const handleServerAction = async (value: {
        title: string
    }) => {
        try {
            ...
            setIsopen(false)
            enqueSnackBar({ type: 'success', message: "Schema Create Successfully!" })
        } catch (error) {
            console.log("error", error)
        }
    }
    return (
        <CustomModal size="md" isOpen={isOpen} title="New Daigram" onClose={() => setIsopen(false)} >
            <Formik
                initialValues={{
                    title: "",
                }}
